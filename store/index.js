...
export const getters = {
  ...
  filteredJeans(state, getters) {
    if(getters.loaded) {
      const genericJeans = state.jeans.filter((object) => !object.exclusive);
      // is the selected shop id inside the exlusive array of the filtered objects ?
      const exclusiveJeans = state.jeans.filter((object) => object.exclusive ? object.exclusive.includes(state.selection.selectedShop.id) : null);
